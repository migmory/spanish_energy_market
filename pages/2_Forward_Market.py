...
# =========================================================
# COUNTRY BASELOAD / SOLAR COMPARISON — ON-DEMAND
# =========================================================
...
def _cc_find_delivery_from_text(text: str) -> str | None:
    ...
    for pat in patterns:
        m = re.search(pat, s)
        if m:
            return normalize_str(m.group(0))
    return None
...
def parse_country_omip_tables_loose(
    ...
    for table_id, table in enumerate(tables, start=1):
        ...
        for _, row in df.iterrows():
            ...
            delivery = None
            for txt in texts:
                delivery = _cc_find_delivery_from_text(txt)
                if delivery:
                    break
            if not delivery:
                continue
            ...
            if d_price is None:
                if len(price_like) >= 2:
                    ...
                    d_price = price_like[-1]
            if d_minus_1 is None and len(price_like) >= 2:
                d_minus_1 = price_like[-1]
            ...
            for txt in texts:
                maybe = extract_contract_name(txt, instrument)
                if maybe:
                    original_contract = maybe
                    break
            contract = original_contract or f"{instrument} {delivery}"
            ...
            rows.append(
                {
                    ...
                    "d_minus_1": d_minus_1,
                    "curve_price": curve_price,
                    "raw_contract_cell": joined[:500],
                    "country": country_name,
                    "zone": zone_code,
                    "variable": sheet_name,
                    "parser": "loose_country_fallback",
                }
            )
    ...
    return out.sort_values(["country", "sheet", "sort_key", "contract"]).reset_index(drop=True)


def country_comparison_pull_one(
    market_date: date,
    product_code: str,
    country_name: str,
    zone_code: str,
...
def country_comparison_chart(df: pd.DataFrame, chart_type: str):
    if df.empty:
        return None
    ...
    base = alt.Chart(plot).encode(
        ...
        tooltip=[
            ...
            alt.Tooltip("d_price:Q", title="D €/MWh", format=",.2f"),
            alt.Tooltip("d_minus_1:Q", title="D-1 €/MWh", format=",.2f"),
            alt.Tooltip("best_bid:Q", title="Best bid", format=",.2f"),
            alt.Tooltip("best_ask:Q", title="Best ask", format=",.2f"),
            alt.Tooltip("market_date:T", title="Market date", format="%Y-%m-%d"),
        ],
    )
...
country_comparison_section_header("🌍 Country forward comparison")
...
with st.expander("Configure country comparison", expanded=False):
    ...
    with cc1:
        cc_market_date = st.date_input(
            "Comparison market date",
            value=date.today(),
            key="cc_market_date",
            help="OMIP publication date used in the URL.",
        )
        cc_product_label = st.selectbox(
            "Product",
            list(PRODUCTS.keys()),
            index=list(PRODUCTS.keys()).index("Power") if "Power" in PRODUCTS else 0,
            key="cc_product_label",
        )

    with cc2:
        default_countries = [x for x in ["Spain", "Portugal", "France", "Germany"] if x in ZONES]
        cc_countries = st.multiselect(
            "Countries / zones",
    ...
    with cc3:
        cc_maturity_label = st.selectbox(
            "Maturity",
            ["Year", "Quarter", "Month", "All"],
            index=0,
        ...
        cc_max_contracts = st.slider(
            ...
            value=10,
            step=1,
            key="cc_max_contracts",
        )
        cc_chart_type = st.radio(
            "Chart type",
            ["Bars", "Lines"],
            index=0,
            horizontal=True,
    ...
    cc_include_maturity = st.checkbox(
        "Send maturity parameter to OMIP URL",
        value=False,
    ...
    cc_run = st.button(
        ...
        key="cc_run_country_comparison",
    )

if cc_run:
    if not cc_countries:
        st.warning("Choose at least one country / zone.")
    else:
        cc_product = PRODUCTS[cc_product_label]
        cc_maturity = MATURITY_FILTERS.get(cc_maturity_label)
        ...
        with st.spinner("Pulling OMIP country comparison curves..."):
            for country in cc_countries:
                ...
                country_data, country_debug, country_raw = country_comparison_pull_one(
                    market_date=cc_market_date,
                    product_code=cc_product,
                ...
                all_debug.extend(country_debug)
                all_raw.update(country_raw)

        cc_debug_df = pd.DataFrame(all_debug)

        if not all_data:
            st.warning(
                "OMIP pages were reachable or attempted, but no comparable rows could be parsed. "
                "Open diagnostics / raw tables below to see exactly what OMIP returned."
            )
            st.dataframe(cc_debug_df, use_container_width=True)

            with st.expander("Raw OMIP tables returned by country comparison", expanded=True):
                if all_raw:
                    for raw_name, raw_df in all_raw.items():
                        st.markdown(f"**{raw_name}**")
                        st.dataframe(raw_df, use_container_width=True)
                else:
                    st.info("No raw tables were returned.")
        else:
            cc_df = pd.concat(all_data, ignore_index=True)
            cc_df = cc_df.dropna(subset=["curve_price"]).copy()

            cc_df = (
                cc_df.sort_values(["country", "variable", "sort_key", "contract"])
                .groupby(["country", "variable"], group_keys=False)
                .head(cc_max_contracts)
                .reset_index(drop=True)
            )

            st.success(
                f"Parsed {len(cc_df):,} contracts across {cc_df['country'].nunique()} countries/zones "
                f"for {cc_market_date:%d-%b-%Y}."
            )

            cc_chart = country_comparison_chart(cc_df, cc_chart_type)
            if cc_chart is not None:
            ...
            cc_display_cols = [c for c in cc_display_cols if c in cc_df.columns]
            cc_table = cc_df[cc_display_cols].rename(
                columns={
                    ...
                    "nr_contracts": "Nr contracts",
                    "session_volume_mwh": "Session volume MWh",
                }
            )
            st.dataframe(styled_df(cc_table), use_container_width=True)

            with st.expander("Country comparison diagnostics", expanded=False):
                st.dataframe(cc_debug_df, use_container_width=True)

            with st.expander("Raw OMIP tables returned by country comparison", expanded=False):
                for raw_name, raw_df in all_raw.items():
                    st.markdown(f"**{raw_name}**")
                    st.dataframe(raw_df, use_container_width=True)
